...
intersect_doPCA_from_file_and_project_second_dataset=function(file,file2,train_string,center=TRUE,scale=FALSE,fread=F) {
  ...
  if(fread==T){
    data1 = fread(file)
    data1 = data1[rowSums((data1[, -1,with=F] == 0)) < ncol(data1[-1]), ] #remove genes with no variance
    data2 = fread(file2)
    ...
    common.genes = glab.library::intersect_all(data1[,1,with=F], data2[,1,with=F])
    data = data1[data1[,1,with=F] %in% common.genes, ]
