...
# vector = geneexp.score.cbc
# title = "cbc score"
# labels = F; ellipse = F; PCx = "PC1"; PCy = "PC2"

# composite$color = composite$rsc.score
# file = paste0(sample.subset.string, ".geneexp_modnames_prcomp_scores.txt")
# vector = composite; labels = F; ellipse = F; PCx = "PC1"; PCy = "PC2"; title = "RSC score"


plot_pca_paint_vector = function(file, vector, title = "", labels = TRUE, PCx="PC1", PCy="PC2", ellipse = F, conf = 0.95, density=F,
                                 fliph = F, flipv = F){  
  ...
  #matched <- intersect(table$Score, vector$sample)
  
  
  table = merge(table, vector, by.x="Score", by.y="sample")
