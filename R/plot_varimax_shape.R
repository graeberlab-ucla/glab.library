...
plot_varimax_shape = function(file, info.name, info.type, info.shape, title = "", labels = TRUE, PCx="PC1", PCy="PC2", ellipse = F, conf = 0.95, density=F,
                    fliph = F, flipv = F){
  ...
  pcx.y <- ggplot(table, aes_string(x=PCx,y=PCy)) +geom_point(size = I(3), aes(color = factor(type), shape = factor(shape))) +
    ...
    guides(color=guide_legend(title="Type"), shape=guide_legend(title="BAP1"))+
    labs(title = title,
         x = paste0(PCx,"", "", ""),
         y = paste0(PCy,"", "", ""))+
