...
#' plot_pca_gradient_fill_repel uses geom_text_repel
#' 
...
#' @export
#' 
plot_pca_gradient_fill_repel = function(file, info.name, info.type, info.color, title = "", labels = TRUE, PCx="PC1", PCy="PC2", ellipse = F, conf = 0.95, density=F,
  ...
  pcx.y <- ggplot(table, aes_string(x=PCx,y=PCy)) +geom_point(size = I(3), aes(fill = color), colour="black",pch=21) +
    ...
    pcx.y <- ggplot(data = subset(table, !is.na(color)), aes_string(x=PCx,y=PCy)) +geom_point(size = I(3), aes(fill = color), colour="black",pch=21) +
      ...
      labs(title = title, 
           ...
           y = paste0(PCy," (", sdev$pve[match(PCy, rownames(sdev))], "%)"))+
      theme_bw(base_size=18)+
      if(labels==TRUE){geom_text_repel(data = subset(table, !is.na(color)), mapping = aes(label = Score), check_overlap = TRUE, size = 3)}
