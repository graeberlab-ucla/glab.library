...
## PCA 3D Visualization with Plotly
...
scores$Group <- info$Group[match(scores$Score, info$cellline)]
scores$Group <- factor(scores$Group, levels = unique(scores$Group))
all(info$cellline==scores$Score)
```

```{r Plot 3d - color by Group}
